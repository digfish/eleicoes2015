        ...
        jQuery.get('{{ $api_url }}',
                function (noticias) {
                    ...
                    if (noticias.length == 0) {
                        console.log('Nao foram encontrados resultados!');
                        $('body > .container').html("<H1>Não foram encontrados resultados!</H1>");
                        return;
                    }
                    ...
                    $('#destaque-principal a.destaque-remove').on('click', function (evt) {
                            evt.preventDefault();
                            delete_noticia_api(destaque_principal.id);
                            $(this).parents('#destaque-principal').remove();
                    });
                    
                    
                    $.each(destaque_principal.tags, function (i, tag) {
                        new_tag = $("<span>").attr( {'class':  'tag label label-success'} ).text( tag.nome );
                        $(new_tag).css('cursor','pointer');
                        $(new_tag).click(function(evt) { window.open("/tag/" + tag.id  ) });
                        
                        $('#destaque-principal .destaque-tags').append(new_tag);
                    });
                    ...
                    $.each(destaques, function (i, destaque) {
                        ...
                        $.each(destaque.tags, function (i, tag) {
                            ...
                            $(new_tag).css('cursor','pointer');
                            $(new_tag).click(function(evt) { window.open("/tag/" + tag.id  ) });
                            $('.destaque-tags', $new_destaque_node).append(new_tag);
                        });
...
<!-- Heading Row -->
<div class="row" id="destaque-principal" class="noticia">
    <div class="col-md-8">
        <a class="link-destaque" href=""><img class="img-responsive img-rounded destaque-imagem" src="" alt=""></a>
    </div>
    <!-- /.col-md-8 -->
    <div class="col-md-4">
        <a class="link-destaque"><h1 class="destaque-titulo"></h1></a>
...
<div class="row destaques">
    <div class="col-md-4 destaque">
        ...
        <a class="btn btn-primary btn-lg destaque-url" href="" target="_blank">Ver notícia</a>
        <p><a class="btn destaque-remove" ><span class="glyphicon  glyphicon-remove"></span> Not&iacute;cia n&atilde;o relevante <a></p>
    </div>
        <!-- /.col-md-4 -->
</div><!-- /.row -->
